...
def correlate_incident(incident_id: str, db: Session = Depends(get_db)):
    ...
    synthesis_row = AnalysisResult(
        ...
        recommended_actions=[a.dict() for a in result.recommended_actions],
        affected_accounts=result.affected_accounts,
        evidence_relationships=[r.dict() for r in result.evidence_relationships],
        timeline_events=[t.dict() for t in result.timeline_events],
